/*
 * O MIOLO PURO DOS WIDGETS — extraído VERBATIM de routes/widgets.js (Fase B, 02/09).
 *
 * Tudo aqui é template e busca de dados: nenhum toque de banco, nenhum req/res. A única
 * linha que tocava o banco (o quadro-espelho lê o widget de origem) virou injeção:
 * usarBuscadorDeWidget(fn) — cada casa liga o seu. Foi extraído para as DUAS casas
 * servirem o MESMO código na Fase B: a rota velha delega para cá, e o lado Nest recebe
 * uma cópia gerada disto. Mudou aqui, ressincronize a cópia de lá.
 */
...
async function seedFor(widget) {
  ...
  try {
    ...
    if (widget.widget_type === 'rss') {
      ...
      const data = await require('./news').getAll(feeds);
      if (!data) return null;
    ...
    if (widget.widget_type === 'lottery') {
      ...
      if (chosen && chosen.length > 1) {
        const many = await require('./lottery').getMany(chosen);
        return many ? { rotation: many } : null;
      }
      return await require('./lottery').getLatest((chosen && chosen[0]) || cfg.game);
    }
...
function renderDirectorySearch(c) {
  c = c || {};
  const src = buscarWidget(c.source_widget_id);
