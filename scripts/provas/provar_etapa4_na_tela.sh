#!/bin/sh
# ETAPA 4 NA TELA - a pagina de Playlists nao lista as listas de contrato.
#
# Roda etapa4_na_tela.js num Chrome de verdade contra o ambiente de teste, com a sessao do
# assinante. Os oraculos sao a API: a tela nao pode dizer o que a API nao diz.
#
# Uso:  TOKEN=<sessao do assinante> sh provar_etapa4_na_tela.sh
...
docker run --rm --network host --user root -v "$(cd "$(dirname "$0")" && pwd):/p" \
  ...
  --entrypoint node zenika/alpine-chrome:with-puppeteer /p/etapa4_na_tela.js
