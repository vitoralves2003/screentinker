...
function limpar() {
  ...
  if (plantado.user) { tenta('DELETE FROM codigos_de_acesso WHERE user_id = ?', plantado.user); tenta('DELETE FROM dispositivos_confiaveis WHERE user_id = ?', plantado.user); }
  if (plantado.ws) { tenta('DELETE FROM workspace_license_daily WHERE workspace_id = ?', plantado.ws); tenta('DELETE FROM workspace_members WHERE workspace_id = ?', plantado.ws); tenta('DELETE FROM workspaces WHERE id = ?', plantado.ws); }
