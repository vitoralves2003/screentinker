#!/bin/sh
# ETAPA 3 NA TELA - o modal de editar arquivo tem so os campos que ficaram, e o servidor nao zera o que saiu.
#
# Roda etapa3_na_tela.js num Chrome de verdade contra o ambiente de teste, com a sessao do
# assinante. Os oraculos sao a API: a tela nao pode dizer o que a API nao diz.
#
# Uso:  TOKEN=<sessao do assinante> sh provar_etapa3_na_tela.sh
...
docker run --rm --network host --user root -v "$(cd "$(dirname "$0")" && pwd):/p" \
  ...
  --entrypoint node zenika/alpine-chrome:with-puppeteer /p/etapa3_na_tela.js
