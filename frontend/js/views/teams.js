...
async function renderList(container) {
  container.innerHTML = `
    <div class="page-header">
      <div><h1>Equipes</h1><div class="subtitle">Gerencie equipes e acesso compartilhado</div></div>
  ...
  try {
    ...
    if (!Array.isArray(teams)) {
      ...
      list.innerHTML = `<div class="empty-state"><h3>${esc('As equipas estão a ser reformuladas')}</h3>`
        + `<p>${esc(teams && teams.message ? teams.message : 'Esta funcionalidade está indisponível enquanto é reformulada para funcionar com as Áreas de trabalho. Os dados das equipas existentes são preservados.')}</p></div>`;
      return;
    }

    if (!teams.length) {
      list.innerHTML = `<div class="empty-state"><h3>Sem equipes ainda</h3><p>Crie uma equipe para compartilhar dispositivos com outros usuários.</p></div>`;
      return;
    }
          ...
          <div style="padding:20px">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
              ...
              <div>
                ...
                <div style="font-size:12px;color:var(--text-muted)">${`Sua função: ${team.my_role}`} &middot; ${`${(team.member_count) === 1 ? `1 membro` : `${team.member_count} membros`}`}</div>
              </div>
            </div>
          </div>
        </div>
...
async function renderTeamDetail(container, teamId) {
  ...
  } catch { container.innerHTML = `<div class="empty-state"><h3>Equipe não encontrada</h3></div>`; return; }
  ...
  container.innerHTML = `
    ...
    <div class="page-header">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn btn-danger btn-sm" id="deleteTeamBtn">Excluir equipe</button>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
      <div class="settings-section" style="margin:0">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <h3 style="font-size:15px">${`Membros (${team.members?.length || 0})`}</h3>
          <button class="btn btn-secondary btn-sm" id="inviteMemberBtn">+ Convidar</button>
        </div>
        <div id="membersList">
          ${(team.members || []).map(m => `
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
              ...
              <select class="input" style="max-width:100px;width:100%;background:var(--bg-input);font-size:12px;padding:4px 8px" data-member-id="${m.user_id}" ${m.role === 'owner' ? 'disabled' : ''}>
                ...
                <option value="owner" ${m.role === 'owner' ? 'selected' : ''}>Proprietário</option>
              </select>
              ${m.role !== 'owner' ? `<button class="btn-icon" data-remove-member="${m.user_id}" style="color:var(--danger)" title="Remover">&#10005;</button>` : ''}
            </div>
          `).join('') || `<p style="color:var(--text-muted);font-size:13px">Sem membros</p>`}
        </div>
      </div>

      <div class="settings-section" style="margin:0">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <h3 style="font-size:15px">${`Dispositivos compartilhados (${devices.length})`}</h3>
        ...
        <div id="teamDevicesList">
          ${devices.map(d => `
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
              ...
              <button class="btn-icon" data-remove-device="${d.id}" style="color:var(--danger)" title="Remover da equipe">&#10005;</button>
            </div>
          `).join('') || `<p style="color:var(--text-muted);font-size:13px">Sem dispositivos compartilhados com esta equipe</p>`}
        </div>
      </div>
    </div>
  `;
  ...
  document.getElementById('inviteMemberBtn').onclick = async () => {
    ...
    if (!email) return;
    const role = prompt('Função (viewer, editor ou owner):', 'editor');
    if (!['viewer', 'editor', 'owner'].includes(role)) { showToast('Função inválida', 'error'); return; }
    try {
  ...
  document.getElementById('addDeviceToTeam').onchange = async (e) => {
    ...
    try {
      ...
      showToast('Dispositivo adicionado à equipe', 'success');
      renderTeamDetail(container, teamId);
    } catch (err) { showToast(err.message, 'error'); }
  };
  ...
  container.querySelectorAll('[data-remove-device]').forEach(btn => {
    btn.onclick = async () => {
      try {
        ...
        showToast('Dispositivo removido da equipe', 'success');
        renderTeamDetail(container, teamId);
      } catch (err) { showToast(err.message, 'error'); }
    };
  });
